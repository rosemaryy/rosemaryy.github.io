...
let roomToItemLstHashMap = new Map();
var visibleNavLst = document.querySelector( '.mdl-list' );

function getNavBarList (panoRoom) {
	// return list of items to be on the navbar for that room
	return roomToItemLstHashMap.get(panoRoom);
}

function setNavBarList (panoRoom, lst){
	// set an entire list for a particular panoroom
	roomToItemLstHashMap.set(panoRoom, lst);
}

function appendToNavBarList (panoRoom, item){
	// set an entire list for a particular panoroom
	panoRoomItemLst = getNavBarList(panoRoom);
	//if lst is empty
	if (typeof panoRoomItemLst == 'undefined') {
		console.log(["Undefined list found for ", panoRoom]);
		setNavBarList(panoRoom, [item]);
	} else {
		panoRoomItemLst.push( item );
	}
}

function setNavBarListInView (panoRoom){
	// sets the navbar list that can be seen in a particular room
	roomNavbarLst = getNavBarList(panoRoom);
	if (typeof panoRoomItemLst !== 'undefined') { 
		for (var i = 0; i < roomNavbarLst.length; i++) {
			visibleNavLst.appendChild(roomNavbarLst[i]);
		}
	}
}

function clearNavBarListInView (){
	// clears the current visible NavBarList
	while (visibleNavLst.hasChildNodes()) {
  		visibleNavLst.removeChild(visibleNavLst.firstChild);
	}
}
function loadVisibleNavBarLst (lst){
	// sets visibleNavLst to point to list that displays menu
	visibleNavLst = lst;
}
...
(function(k, d) {
    ...
    r.prototype = Object.assign(Object.create(d.Mesh.prototype), {
        ...
        link: function(a, b, c, e) {
            ...
            e.position.copy(b),
            e.toPanorama = a,
            e.addEventListener("click", function() {  // TODO: SET UP EVENT LISTENER FOR LIST CHANGING HERE
                this.dispatchEvent({
                    type: "panolens-viewer-handler",
                    method: "setPanorama",
                    data: a
        ...
        linkWtMenuChange: function(a, b, c, e) {
            this.visible = !0;
            b ? (c = void 0 !== c ? c : void 0 !== a.linkingImageScale ? a.linkingImageScale : 300,
            e = e ? e : a.linkingImageURL ? a.linkingImageURL : t.Arrow,
            e = new y(c,e),
            ...
            e.addEventListener("click", function() {  // TODO: SET UP EVENT LISTENER FOR LIST CHANGING HERE
                this.dispatchEvent({
                    type: "panolens-viewer-handler",
                    method: "setPanoramaWtMenuChange",
                    data: a
                })
            }
            .bind(this)),
    ...
    ea.prototype = Object.assign(Object.create(d.EventDispatcher.prototype), {
        ...
        setPanoramaWtMenuChange: function(a) {
            var b = this.panorama;
            clearNavBarListInView();
            // a is the new panorama
            setNavBarListInView(a); // todo: complete
